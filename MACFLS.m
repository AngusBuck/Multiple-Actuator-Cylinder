...
%Clear all variables
% clearvars
%If you want to compare a variable between runs, 'save' it/them as (say) keepwx = w_x, keepq = q, keepAx = A_x and use
%clearvars -except keepwx keepq keepDx

%%%user inputs required%%%
n = 64;                                 %number of evaluation points per turbine (code is O(n^2)) (must be a multiple of four)

T = 5;                                  %number of turbines (code is O(T^2))
...
c = 0.128;                              %The chord length planned for VAWT MRS, where R=8.865
...
% for I = 1:T
%     fplot(@(t) R*sin(t) + spac*(I-1)*sin(gamma*pi/180)*2*R, @(t) R*cos(t) + spac*(I-1)*cos(gamma*pi/180)*2*R)
...
%%%%%%Pretty sure the below is equivalent to the above, and thus unneeded
% % for I2 = Teval
% %     for i2 = 1:n
% %         %Power extracted from wind = rho*U_0^2*integral[u(theta)*q(theta)*R]d(theta) ~= rho*U_0^2*sum{ u(theta_i)*q(theta_i)*R*deltaTheta }
% %         WindPowerSeries(i2,I2) = -(1.225*U_0^2)*U_0*( w_y(i2,I2)*cos(thetaval(i2)) - (1+w_x(i2,I2))*sin(thetaval(i2)) ) *q(i2,I2) *R*deltatheta;
% %         %This is just summed because the actuator cylinder is assumed to extract energy from accross the entire cylinder
% %         %This is the upper limit of VAWT energy generation; unclear to me if this is the same as a Betz limit or not
% % 
% %         %Power generated by turbine = omega*B/2pi*integral[R*T(th)d(th)] ~= (TSR*U_0/R)*B/2pi*R*sum{R*Ct(theta)*0.5*rho*W(theta)^2*chord}
% %         GeneratedPowerSeries(i2,I2) = (TSR(I2)*U_0/R*B/2/pi)*R*( Cl(i2,I2)*sin(Phi(i2,I2))-Cd(i2,I2)*cos(Phi(i2,I2)) )*0.5*1.225*W2(i2,I2)*c/(cos(0))*deltatheta;
% %         %This is averaged (sum/2pi) because all the power is being generated at one point at any instant (where the blade is), but it extracts different power at different points through a revolution (also hence *B for B blades)
% %     end
% % end

%%%%%%I should put Ct back in, bc I can use that for thrust
%%%%%%reasons (see R4A) (also the power eqn)
...
%%%%%%                        %I know I know, I hate this tiny 10^-4 offset too, but it stops the code giving orange integration errors
                    ...
                    elseif -1<=y_i(i,j,I,J) && y_i(i,j,I,J) <=1 && x_i(i,j,I,J) >=0 && x_i(i,j,I,J)^2 + y_i(i,j,I,J)^2 >=1
                        ...
                        %%%%%%[~,indexJk] = min(thetaJkTest) is equivalent, probably??
